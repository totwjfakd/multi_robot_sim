...
Fleet Command GUI
-----------------
특정 로봇과 목적지 노드 ID를 선택해 fleet_commander로 이동 명령을 전송한다.
"""
...
class FleetCommandGuiNode(Node):
    def __init__(self):
        ...
        self.node_coords, self.edge_pairs = self._load_graph(self.get_parameter('graph_file').value)
    ...
    def _load_graph(self, graph_file: str):
        ...
        for feat in data.get('features', []):
            ...
            if geometry_type == 'Point':
                try:
                    node_id = int(feat['properties']['id'])
        ...
        for start_id, end_id in edges:
            ...
            if key in seen_edges or start_id not in node_ids or end_id not in node_ids:
                continue
            seen_edges.add(key)
            unique_edges.append((start_id, end_id))

        return sorted(nodes, key=lambda item: item[0]), unique_edges
...
class FleetCommandGuiApp:
    ...
    def _build_layout(self):
        ...
        self.node_combo.bind('<<ComboboxSelected>>', self._on_node_text_changed)
        ...
        send_button.grid(row=3, column=0, columnspan=2, sticky='ew')
    ...
    def _on_node_text_changed(self, _event=None):
        self._mark_graph_dirty()
    ...
    def _focus_table_node(self, node_id: int):
        ...
        current_selection = self.node_table.selection()
    ...
    def _draw_edges(self, transform: dict):
        ...
        for start_id, end_id in self.node.edge_pairs:
            if start_id not in self.node.node_map or end_id not in self.node.node_map:
                continue
    ...
    def _closest_node_from_canvas(self, canvas_x: float, canvas_y: float):
        ...
        click_radius = (self.NODE_RADIUS + 10) ** 2
        if closest_dist_sq > click_radius:
            return None
